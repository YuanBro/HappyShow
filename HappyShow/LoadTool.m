...
@synthesize TAG;
static LoadTool *sharePhotoTool = nil;
...
-(NSMutableArray<ModelPhoto*>*)photoAblumList
{
    if (_photoAblumList.count==0) {
        _photoAblumList = [NSMutableArray array];
    }

    return _photoAblumList;
}

#pragma mark - list albums
- (NSMutableArray<ModelPhoto*> *)getPhotoAblumList
{
...
- (void)requestImageForAsset:(PHAsset *)asset size:(CGSize)size resizeMode:(PHImageRequestOptionsResizeMode)resizeMode completion:(void (^)(UIImage *, NSDictionary *))completion
{
    ...
    CGFloat width = MIN(kScreen_width, kMaxImageWidth);
